...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.updateCurrentJob = function (rpcData) {
        ...
        //记录下最后的job
        var lastJobid=parseInt(tmpBlockTemplate.jobId,16)-1;//这玩意16进制的
        var lastJobArray={};
        for(var i=0;i<3;i++)
        {
            if((lastJobid-i)>0){
                var key=(lastJobid-i).toString(16);
                if(this.validJobs[key])
                {
                    lastJobArray[key]=this.validJobs[key];
                    console.log("lastJob "+key+"");
                }
            }
        }
